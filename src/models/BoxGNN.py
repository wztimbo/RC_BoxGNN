...
class BoxGNN(BaseModel):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument("--inverse_r", action="store_false", default=True, 
        ...
        parser.add_argument('--test_all', type=int, default=1,
                            help='Whether to test all dataset')
        return parser

    def __init__(self, args, corpus):
        #corpus: BoxGNNReader entity
        self.item_num=1 
        self.test_all=args.test_all
        self.model_path=args.model_path
        super().__init__(args,corpus)

        # train_data, val_data, test_data, user2tag, item2tag, data_dict, data_stat = corpus.read_files()
        # adj_cf_mat, norm_cf_mat, mean_cf_mat = corpus.build_interact_matrix(train_data, data_stat)
        # adj_mat, norm_mat, mean_mat = corpus.build_graph(train_data, user2tag, item2tag, data_stat)

        data_stat=corpus.data_stat
        item2tag=corpus.data_gnn['item2tag']
        user2tag=corpus.data_gnn['user2tag']
