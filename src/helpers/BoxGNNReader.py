from helpers.BaseReader import *
...
class BoxGNNReader(BaseReader):
    ...
    def __init__(self, args):
        ...
        self.inverse_r=args.inverse_r
        self.data_gnn,self.data_dict, self.data_stat=self._read_data_GNN()
        self.item_meta_df=pd.DataFrame()
        self._read_data()
        super().__init__(args)
    ...
    def build_graph(self, train_data, user2tag, item2tag, data_stat):
        ...
        # users, items, tags = train_data["user_id"].to_numpy(), user_tag_item["item_id"].to_numpy(), user_tag_item["tagID"].to_numpy()
...
#构建连接矩阵
    def build_interact_matrix(self, train_data, data_stat):
        ...
        #调用归一化函数
        norm_mat = _bi_norm_lap(cf_adj)  #双向（对称）归一化
        mean_mat = _si_norm_lap(cf_adj)  #行归一化
    ...
    def _read_data(self):
        logging.info('*******BOXGNN Reading data from \"{}\", dataset = \"{}\" '.format(self.prefix, self.dataset))
        self.data_df = dict()
        for key in ['train', 'dev', 'test']:
            self.data_df[key] = pd.read_csv(os.path.join(self.prefix, self.dataset, key + '.txt'), sep='\t').reset_index(drop=True).sort_values(by = ['user_id'])
    ...
    def  _read_data_GNN(self):
        print("reading files ...")
        data_path = os.path.join(self.path, self.dataset)
        train_file = os.path.join(data_path, "train.txt")
        val_file = os.path.join(data_path, "dev.txt")
        test_file = os.path.join(data_path, "test.txt")
        user2tag_file = os.path.join(data_path, "user2tag.txt")
        item2tag_file = os.path.join(data_path, "item2tag.txt")
        
        train_data = pd.read_csv(train_file, sep="\t")[["user_id", "item_id"]].drop_duplicates().reset_index(drop=True)
        ...
        return data_gnn,data_dict,data_stat
